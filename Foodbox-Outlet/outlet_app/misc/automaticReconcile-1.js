...
module.exports.InitAutomaticReconcile = function () {
    console.log("InitAutomaticReconcile function called");
...
function GetPODetails(is_end_of_day) {
    var reconcile_items = [];
    ...
    request({
        ...
        method: "GET"
               ...
               function (error, response, data) {
                   ...
                   {
                       console.log("AutomaticReconcile function :: get_po_details Error:: ################################################### Error:: " + error);
                       ...
                       for (var po_id in json_parsed_po_in_redis)
                       {
                           ...
                           for (var item_count = 0; item_count < po_items.length; item_count++)
                           {
                               ...
                               if (reconcile_stock_item_data.length > 0)
                               {
                                   ...
                                   console.log("********************************************* scanned_item_count :: " + scanned_item_count);
                   ...
                   save_reconcile_data_main(reconcile_items, excess_quantity_items, function (err, res) {
                       if (err)
                       {
                           console.error('save_reconcile_data :' + err);
                       }
                       else
                       {
                           send_restatrant_excess_po_mail_main(reconcile_items, excess_quantity_items, function (mailerr, res1) {
                               if (mailerr)
                               {
                                   console.error('save_reconcile_data :' + err);
                               }
                               ...
                               {
                                   update_received_time(reconcile_items, excess_quantity_items, function (err, res) {
                                       if (err)
                                       {
                                           console.error('update_received_time: ' + err);
                                       }
                                       else
                                       {
                                           update_reconcile_stock_count_automatic_main(reconcile_items, excess_quantity_items, function (mailerr, res1) {
                                               if (err)
                                               {
                                                   console.error('update_reconcile_stock_count_automatic_main: ' + err);
                                               }
                                               else
                                               {
                                                   console.log('Automatic reconcile functionality completed');
                                                   return;
                                               }
                                           });
                                       }
...
function save_reconcile_data_main(reconcile_items, excess_quantity_items, callback) {
    console.log("AutomaticReconcile function :: save_reconcile_data:: function started ################################################### reconcile_items:: " + JSON.stringify(reconcile_items));

    request({
        url: outlet_url + '/outlet_app/save_reconcile_data',
        method: "POST",
        json: { "reconcile_items": JSON.stringify(reconcile_items) }
    },
    function (error, response, data) {
        if (error || (response && response.statusCode != 200))
        {
        ...
        console.log("AutomaticReconcile function :: save_reconcile_data:: function end ################################################### reconcile_items:: " + JSON.stringify(reconcile_items));
        callback(null, "Success");
    });
}


function send_restatrant_excess_po_mail_main(reconcile_items, excess_quantity_items, callback) {
    console.log("AutomaticReconcile function :: send_restatrant_excess_po_mail:: function started ################################################### excess_quantity_items:: " + JSON.stringify(excess_quantity_items));
    // excess item details mail send to respective restaurant
    send_restatrant_excess_po_mail(excess_quantity_items);
    console.log("AutomaticReconcile function :: send_restatrant_excess_po_mail:: function end ################################################### excess_quantity_items:: " + JSON.stringify(excess_quantity_items));
    callback(null, "Success");
}


function update_received_time(reconcile_items, excess_quantity_items, callback) {
    console.log("AutomaticReconcile function :: IncomingPOProcess::  function started ################################################### reconcile_items:: " + JSON.stringify(reconcile_items));
...
function update_reconcile_stock_count_automatic_main(reconcile_items, excess_quantity_items, callback) {
    console.log("AutomaticReconcile function :: update_reconcile_stock_count_automatic::  function started ################################################### reconcile_items:: " + JSON.stringify(reconcile_items));
    request({
    ...
    function (error, response, data) {
        ...
        console.log("AutomaticReconcile function :: update_reconcile_stock_count_automatic::  function end ################################################### reconcile_items:: " + JSON.stringify(reconcile_items));
        reconcile_items = [];
